...
def fetch_description(gh: Any, relative_url: str) -> str:
    ...
    try:
        ...
        if len(url_parts) < 3:
            click.echo(
                f"Warning: Invalid relative URL format: {relative_url}", err=True
            )
            return ""
        ...
        if not owner or not repository:
            click.echo(
                f"Warning: Empty owner or repository in URL: {relative_url}", err=True
            )
            return ""
        ...
        try:
            ...
            click.echo(
                f"Warning: Failed to fetch repository {owner}/{repository}: {e}",
                err=True,
            )
            return ""
        ...
        if repo_obj.description:
            repo_description = textwrap.shorten(
                repo_obj.description, width=60, placeholder="..."
            )
        return repo_description

    except Exception as e:
        click.echo(
            f"Warning: Unexpected error fetching description for {relative_url}: {e}",
            err=True,
        )
        return ""
...
def show_result(
    ...
    if table:
        if repos:
            ...
            click.echo(
                f"found {total_repos_count} {destinations} others {destinations} are private"
            )
            click.echo(
                f"found {more_than_zero_count} {destinations} with more than zero star"
            )
        else:
            click.echo(f"Doesn't find any {destinations} that match search request")
    else:
        click.echo(json.dumps(repos))


def get_max_deps(
    sess: requests.Session, url: str, timeout: int = REQUEST_TIMEOUT
) -> int:
    """
    ...
    try:
        ...
        if not deps_count_element:
            click.echo(
                "Error: Could not find dependents count element in page", err=True
            )
            click.echo(
                "The page structure may have changed or the URL is invalid", err=True
            )
            sys.exit(1)
        ...
        click.echo(
            f"Error: Unexpected error while parsing dependents count: {e}", err=True
        )
        sys.exit(1)
...
def validate_github_url(url: str) -> tuple[str, str]:
    ...
    if parsed.netloc and parsed.netloc not in ["github.com", "www.github.com"]:
        click.echo(
            f"Error: URL must be a GitHub repository URL (github.com), got: {parsed.netloc}",
            err=True,
        )
        sys.exit(1)
    ...
    if len(path_segments) != 2:
        ...
        click.echo("Expected format: https://github.com/owner/repository", err=True)
        click.echo(
            f"Got {len(path_segments)} path segment(s): {'/'.join(path_segments)}",
            err=True,
        )
        sys.exit(1)
    ...
    if not valid_pattern.match(owner):
        click.echo(
            f"Error: Invalid owner name '{owner}' - must contain only alphanumeric characters, dots, hyphens, or underscores",
            err=True,
        )
        sys.exit(1)
    ...
    if not valid_pattern.match(repository):
        click.echo(
            f"Error: Invalid repository name '{repository}' - must contain only alphanumeric characters, dots, hyphens, or underscores",
            err=True,
        )
        sys.exit(1)
...
def cli(
    ...
    if report and not base_url:
        click.echo(
            "Error: GHTOPDEP_BASE_URL environment variable is required for report mode",
            err=True,
        )
        sys.exit(1)
    ...
    if report:
        report_url = f"{base_url}/repos/{owner}/{repository}"
        try:
            ...
            if result.status_code == 200:
                try:
                    ...
                    click.echo(
                        f"Error: Invalid response format from report server: {e}",
                        err=True,
                    )
                    sys.exit(1)
                ...
                click.echo(
                    f"Error: Report server returned status {result.status_code}",
                    err=True,
                )
                sys.exit(1)
            ...
            click.echo(
                f"Error: Could not connect to report server ({base_url}): {e}", err=True
            )
            sys.exit(1)
        except requests.exceptions.HTTPError as e:
    ...
    # Initialize the first page URL for scraping dependents
    page_url: str | None = (
        f"{url}/network/dependents?dependent_type={destination.upper()}"
    )

    # page_url is guaranteed to be str at this point (f-string literal)
    if (
        not page_url
    ):  # pragma: no cover - Type narrowing for mypy, impossible to trigger
        click.echo(
            "Error: Failed to construct initial page URL", err=True
        )  # pragma: no cover
    ...
    while True:
        ...
        if page_count > MAX_PAGES:
            click.echo(
                f"Warning: Reached maximum page limit ({MAX_PAGES}), stopping pagination",
                err=True,
            )
            break
        ...
        try:
            ...
            click.echo(
                f"Warning: Request timeout on page {page_count}, stopping pagination",
                err=True,
            )
            break
        except requests.exceptions.ConnectionError as e:
        ...
        try:
            ...
            click.echo(
                f"Warning: Failed to parse HTML on page {page_count}: {e}", err=True
            )
            break
            ...
            for dep in dependents:
                try:
                    ...
                    try:
                        ...
                        click.echo(
                            f"Warning: Could not parse star count '{repo_stars}'",
                            err=True,
                        )
                        continue
                    ...
                    if repo_stars_num >= minstar:
                        ...
                        try:
                            relative_repo_url = repo_selector_list[0].attributes.get(
                                "href"
                            )
                            ...
                            click.echo(
                                "Warning: Could not extract repository URL", err=True
                            )
                            continue

                        repo_url = f"{GITHUB_URL}{relative_repo_url}"
                    ...
                    click.echo(
                        f"Warning: Error processing dependent on page {page_count}: {e}",
                        err=True,
                    )
                    continue
        ...
        try:
            ...
            if len(pagination_buttons) == 2:
                ...
                len(pagination_buttons) == 0
                or pagination_buttons[0].text() == "Previous"
            ):
                break
            else:
                break
            ...
            click.echo(
                f"Warning: Error reading pagination controls on page {page_count}: {e}",
                err=True,
            )
            break
    ...
    if report:
        report_post_url = f"{base_url}/repos"
        try:
            payload = {
                "url": url,
                "owner": owner,
            ...
            if response.status_code not in [200, 201]:
                click.echo(
                    f"Error: Report server returned status {response.status_code}",
                    err=True,
                )
                sys.exit(1)
            ...
            click.echo(
                f"Error: Report server POST request timeout ({report_post_url})",
                err=True,
            )
            sys.exit(1)
        except requests.exceptions.ConnectionError as e:
            click.echo(
                f"Error: Could not connect to report server to submit results ({base_url}): {e}",
                err=True,
            )
            sys.exit(1)
        except requests.exceptions.HTTPError as e:
    ...
    if search:
        if gh is None:
            click.echo(
                "Error: GitHub token required for search functionality", err=True
            )
            sys.exit(1)
        for repo in repos:
            try:
                ...
                if not repo_path:
                    click.echo(
                        f"Warning: Could not extract path from repo URL: {repo['url']}",
                        err=True,
                    )
                    continue

                try:
                    search_query = f"{search} repo:{repo_path}"
                    ...
                    for s in search_results:
                        try:
                            if hasattr(s, "html_url"):
                                click.echo(
                                    "{} with {} stars".format(s.html_url, repo["stars"])
                                )
                            else:
                                click.echo(
                                    "Warning: Search result missing html_url attribute",
                                    err=True,
                                )
                        except Exception as e:
                            click.echo(
                                f"Warning: Error displaying search result: {e}",
                                err=True,
                            )
                            continue
                    ...
                    click.echo(
                        f"Warning: GitHub API search failed for {repo_path}: {e}",
                        err=True,
                    )
                    continue
                ...
                click.echo(
                    f"Warning: Error processing search for repository: {e}", err=True
                )
                continue
    else:
        show_result(
