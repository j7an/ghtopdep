...
import time
from ghtopdep.cli import humanize, already_added, sort_repos, readable_stars


class TestHumanize:
    """Tests for the humanize function."""

    def test_humanize_under_1000(self):
    ...
    def test_humanize_1000_to_10000(self):
    ...
    def test_humanize_10000_to_1000000(self):
    ...
    def test_humanize_over_1000000(self):
...
class TestAlreadyAdded:
    ...
    def test_already_added_not_found(self):
    ...
    def test_already_added_url_case_sensitive(self):
...
class TestSortRepos:
    ...
    def test_sort_repos_basic(self, sample_repos):
    ...
    def test_sort_repos_limit_rows(self, sample_repos):
    ...
    def test_sort_repos_empty_list(self):
        """Test with empty repository list."""
        result = sort_repos([], 10)
        assert result == []

    def test_sort_repos_rows_greater_than_list(self, sample_repos):
        """Test when rows requested is greater than list size."""
        result = sort_repos(sample_repos, 100)
        assert len(result) == 3

    def test_sort_repos_zero_rows(self, sample_repos):
        """Test with zero rows requested."""
        result = sort_repos(sample_repos, 0)
        assert result == []

    def test_sort_repos_ties_preserved(self):
...
class TestReadableStars:
    ...
    def test_readable_stars_converts_numbers(self, sample_repos):
    ...
    def test_readable_stars_preserves_other_fields(self, sample_repos):
        """Test that other fields are preserved."""
        result = readable_stars(sample_repos)
        assert "url" in result[0]
        assert result[0]["url"] == "https://github.com/user1/repo1"

    def test_readable_stars_empty_list(self):
        """Test with empty list."""
        result = readable_stars([])
        assert result == []

    def test_readable_stars_multiple_formats(self):
...
class TestSetBasedDuplicateDetection:
    ...
    def test_set_based_duplicate_detection_with_duplicates(self):
    ...
    def test_set_based_with_exclusion_logic(self):
...
class TestPerformanceBenchmark:
    ...
    def test_set_based_performance_benchmark(self):
        """
        ...
        assert set_time < list_time / 5, f"Set-based ({set_time}s) should be significantly faster than list-based ({list_time}s)"

    def test_scalability_set_vs_list(self):
        """
        ...
        assert time_ratio < 100, f"Set-based lookup should scale linearly with operations, not collection size. Ratio: {time_ratio}x"
